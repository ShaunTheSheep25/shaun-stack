...
As this week mainly focused on the containerization + CI/CD pipelining aspects of the services, apart from the pre-commit checks + the checks that are run on each pull/push, there were no other tests that could be run on the code. The detailed commands for using the Makefile tests have been provided above.
...
- Containers run as root, but this is not suitable for production (detailed in SECURITY.md), so I'd like to look into isolating them later on without depending on root
- SQLite is used for persistence, but is usually not considered for multi-instance production deployments, so I'd use PostgreSQL next time for scalability and production-level development.
- No resource limits set on containers, so ideally I'd set a bound on that to keep things tight.
- sentinel-events event store is in-memory and lost on restart, so ideally storing that in a database volume would be ideal.
